        ...
        if (line->type == INSN_LINE) {
            switch (line->insn_line.insn.type) {
                ...
                case REGIMM:
                    switch (line->insn_line.insn.insn) {
                        case ADDI:
                            printf("%012b", ADDI_FUNCT7);                   // 31-25
                            printf("%s", reg2code(line->insn_line.rs1)); // 24-20
...
int asm_line_free(void* asm_line) {
    asm_line_t* line = (asm_line_t*)asm_line;
    if (line->type == INSN_LINE) {
        if (&(line->insn_line.label) != NULL) string_free(&(line->insn_line.label));
    }
    else {
        if (&(line->label.name)      != NULL) string_free(&(line->label.name));
    }
    free(line);
    return 0;
}